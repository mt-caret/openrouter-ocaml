  ...
  Command.async_or_error
    ...
    (let%map_open.Command model =
     ...
     fun () ->
       ...
       if list_models
       then (
         let%bind response = Openrouter_api.list_models ~api_key in
         match response with
         | Error error ->
           eprintf "Error fetching models: %s\n" (Error.to_string_hum error);
         ...
         match message with
         | None ->
           eprintf "Usage: openrouter_cli [options] MESSAGE\n";
           eprintf "       openrouter_cli --list-models\n";
           Deferred.Or_error.return ()
         | Some message ->
           (* One-shot mode *)
