...
let embeddings_url = Uri.of_string "https://openrouter.ai/api/v1/embeddings"
...
let embeddings ~api_key (request : Embeddings.Request.t) =
  let headers = make_headers ~api_key in
  let body =
    [%jsonaf_of: Embeddings.Request.t] request
    |> Jsonaf.to_string
    |> Cohttp_async.Body.of_string
  in
  let%bind response, body = Cohttp_async.Client.post ~headers ~body embeddings_url in
  let%map body_string = Cohttp_async.Body.to_string body in
  let%bind.Or_error () =
    if is_success_status response
    then Ok ()
           ...
           (lazy
             [%message
               "Failed to parse response body into JSON"
                 ...
                 (body_string : string)])
  in
  Or_error.try_with (fun () -> [%of_jsonaf: Embeddings.Response.t] json)
  |> Or_error.tag_s_lazy
       ~tag:
         (lazy
           [%message
             "Failed to parse JSON into Embeddings.Response.t"
               (response : Cohttp.Response.t)
               (json : Jsonaf.t)])
;;
