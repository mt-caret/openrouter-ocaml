...
module Request : sig
  module Message : sig
    ...
    module Content_part : sig
      ...
      (** Create an image part from base64-encoded data. [mime_type] should be e.g.
          "image/jpeg", "image/png", "image/webp", "image/gif" *)
      val image_base64 : mime_type:string -> data:string -> t

      (** Create a file part. [file_data] can be either a URL (for publicly accessible
          files) or a data URL like "data:application/pdf;base64,..." for local files *)
...
val chat : api_key:string -> Request.t -> Response.t Or_error.t Deferred.t
...
val list_models : api_key:string -> Models_response.t Or_error.t Deferred.t

val embeddings
  :  api_key:string
  -> Embeddings.Request.t
  -> Embeddings.Response.t Or_error.t Deferred.t
